...
_rig_binding_free (void *object)
{
  ...
  g_slice_free (RigBinding, binding);
}
...
{
  RutType *type = &rig_binding_type;
#define TYPE RigBinding

  rut_type_init (type, G_STRINGIFY (TYPE));
  rut_type_add_refable (type, ref_count, _rig_binding_free);
...
generate_function_node (RigBinding *binding)
{
  ...
  rut_refable_unref (binding->function_node);
                            ...
                            const char *expression)
{
  ...
  rut_refable_unref (binding->expression_node);
