...
static inline int
rig_protobuf_c_data_buffer_fragment_avail (ProtobufCDataBufferFragment *frag)
{
  return PROTOBUF_C_FRAGMENT_DATA_SIZE - frag->buf_start - frag->buf_length;
}
static inline uint8_t *
rig_protobuf_c_data_buffer_fragment_start (ProtobufCDataBufferFragment *frag)
{
 ...
 */
void
rig_protobuf_c_data_buffer_init(ProtobufCDataBuffer *buffer,
                                ProtobufCAllocator *allocator)
{
 ...
 */
void
rig_protobuf_c_data_buffer_append(ProtobufCDataBuffer *buffer,
                                  const void *data,
                                  size_t length)
{
  ...
  while (length > 0)
      ...
      if (!buffer->last_frag)
	{
	  ...
	  avail = rig_protobuf_c_data_buffer_fragment_avail (buffer->last_frag);
	}
      else
	{
	  avail = rig_protobuf_c_data_buffer_fragment_avail (buffer->last_frag);
      ...
      if (avail > length)
	avail = length;
      memcpy (rig_protobuf_c_data_buffer_fragment_end (buffer->last_frag), data, avail);
...
void
rig_protobuf_c_data_buffer_append_repeated_char (ProtobufCDataBuffer *buffer,
                                                 char character,
                                                 size_t count)
{
  CHECK_INTEGRITY (buffer);
  ...
  while (count > 0)
      ...
      else
	{
      ...
      if (avail > count)
	avail = count;
      memset (rig_protobuf_c_data_buffer_fragment_end (buffer->last_frag), character, avail);
      count -= avail;
      buffer->last_frag->buf_length += avail;
    }
  CHECK_INTEGRITY (buffer);
...
#if 0
void
rig_protobuf_c_data_buffer_append_repeated_data (ProtobufCDataBuffer    *buffer,
 ...
 */
void
rig_protobuf_c_data_buffer_append_string(ProtobufCDataBuffer *buffer,
                                         const char *string)
{
  assert (string != NULL);
 ...
 */
void
rig_protobuf_c_data_buffer_append_char(ProtobufCDataBuffer *buffer,
                                       char character)
{
 ...
 */
void
rig_protobuf_c_data_buffer_append_string0 (ProtobufCDataBuffer *buffer,
                                           const char *string)
{
 ...
 */
size_t
rig_protobuf_c_data_buffer_peek (const ProtobufCDataBuffer *buffer,
                                 void *data,
                                 size_t max_length)
{
  int rv = 0;
  ...
  while (max_length > 0 && frag)
	...
	{
	  memcpy (data, rig_protobuf_c_data_buffer_fragment_start (frag), frag->buf_length);
	  rv += frag->buf_length;
	...
	{
	  memcpy (data, rig_protobuf_c_data_buffer_fragment_start (frag), max_length);
	  rv += max_length;
	  data = (char *) data + max_length;
	  max_length = 0;
	}
 ...
 */
char *
rig_protobuf_c_data_buffer_read_line(ProtobufCDataBuffer *buffer)
{
  int len = 0;
  char *rv;
 ...
 */
char *
rig_protobuf_c_data_buffer_parse_string0(ProtobufCDataBuffer *buffer)
{
 ...
 */
int
rig_protobuf_c_data_buffer_peek_char(const ProtobufCDataBuffer *buffer)
{
  const ProtobufCDataBufferFragment *frag;
  ...
  for (frag = buffer->first_frag; frag; frag = frag->next)
    if (frag->buf_length > 0)
      break;
  return * rig_protobuf_c_data_buffer_fragment_start ((ProtobufCDataBufferFragment*)frag);
}
 ...
 */
int
rig_protobuf_c_data_buffer_read_char (ProtobufCDataBuffer *buffer)
{
  char c;
 ...
 */
int
rig_protobuf_c_data_buffer_writev (ProtobufCDataBuffer *read_from,
                                   int fd)
{
  int rv;
  struct iovec *iov;
...
int
rig_protobuf_c_data_buffer_writev_len (ProtobufCDataBuffer *read_from,
                                       int fd,
                                       size_t max_bytes)
{
  int rv;
...
/* TODO: zero-copy! */
int
rig_protobuf_c_data_buffer_read_in_fd (ProtobufCDataBuffer *write_to,
                                       int read_from)
{
  char buf[8192];
 ...
 */
int
rig_protobuf_c_data_buffer_index_of(ProtobufCDataBuffer *buffer,
                                    char       char_to_find)
{
...
size_t
rig_protobuf_c_data_buffer_drain (ProtobufCDataBuffer *dst,
                                  ProtobufCDataBuffer *src)
{
  ...
  for (frag = src->first_frag; frag; frag = frag->next)
    rig_protobuf_c_data_buffer_append (dst,
...
#if GSK_DEBUG_BUFFER_ALLOCATIONS
size_t
rig_protobuf_c_data_buffer_transfer (ProtobufCDataBuffer *dst,
                                     ProtobufCDataBuffer *src,
                                     size_t max_transfer)
{
  size_t rv = 0;
  ...
  for (frag = src->first_frag; frag && max_transfer > 0; frag = frag->next)
    {
      ...
      if (len >= max_transfer)
        {
          rig_protobuf_c_data_buffer_append (dst, rig_protobuf_c_data_buffer_fragment_start (frag), max_transfer);
          rv += max_transfer;
          break;
        }
      else
        {
          rig_protobuf_c_data_buffer_append (dst, rig_protobuf_c_data_buffer_fragment_start (frag), len);
          rv += len;
          max_transfer -= len;
        }
    }
 ...
 */
void     rig_protobuf_c_data_buffer_printf              (ProtobufCDataBuffer    *buffer,
					 const char   *format,
					 ...)
{
  va_list args;
 ...
 */
int
rig_protobuf_c_data_buffer_polystr_index_of    (ProtobufCDataBuffer    *buffer,
                                char        **strings)
{
