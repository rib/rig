 ...
 *   list owned by a RigCamera must add at least one link into the
...
{
  rig_display_list_unsplice (sub_list);
...
{
  rig_display_list_unsplice (list);
  ...
  for (l = display_list->head; l; l = l->next)
    {
      ...
      switch (cmd->type)
        {
        case RIG_CMD_TYPE_NOP:
          continue;
        case RIG_CMD_TYPE_TRANSFORM_PUSH:
          cogl_framebuffer_push_matrix (fb);
          break;
        case RIG_CMD_TYPE_TRANSFORM_POP:
          cogl_framebuffer_pop_matrix (fb);
          break;
        case RIG_CMD_TYPE_TRANSFORM:
          {
            RigTransformCmd *transform_cmd = RIG_TRANSFORM_CMD (cmd);
            cogl_framebuffer_transform (fb,
                                        &transform_cmd->matrix);
            break;
          }
        case RIG_CMD_TYPE_PRIMITIVE:
          {
            RigPrimitiveCmd *prim_cmd = RIG_PRIMITIVE_CMD (cmd);
            cogl_framebuffer_draw_primitive (fb,
                                             prim_cmd->pipeline,
                                             prim_cmd->primitive);
            break;
          }
        case RIG_CMD_TYPE_TEXT:
          {
            RigTextCmd *text_cmd = RIG_TEXT_CMD (cmd);
        ...
        case RIG_CMD_TYPE_RECTANGLE:
          {
            RigRectangleCmd *rect_cmd = RIG_RECTANGLE_CMD (cmd);
