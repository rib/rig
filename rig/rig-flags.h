...
#ifndef __RIG_FLAGS_H
#define __RIG_FLAGS_H
...
/* The bits are stored in an array of unsigned longs. To use these
   ...
   using RIG_FLAGS_N_LONGS_FOR_SIZE, eg:

   typedef enum { FEATURE_A, FEATURE_B, FEATURE_C, N_FEATURES } Features;

   unsigned long feature_flags[RIG_FLAGS_N_LONGS_FOR_SIZE (N_FEATURES)];
*/

#define RIG_FLAGS_N_LONGS_FOR_SIZE(size)       \
  (((size) +                                    \
 ...
 * this:
 *
 * int n_longs = RIG_FLAGS_N_LONGS_FOR_SIZE (...);
 * unsigned long flags[n_longs];
 * int bit_num;
 *
 * RIG_FLAGS_FOREACH_START (flags, n_longs, bit_num)
 *   {
 *     do_something_with_the_bit (bit_num);
 *   }
 * RIG_FLAGS_FOREACH_END;
 */
#define RIG_FLAGS_FOREACH_START(array, n_longs, bit)   \
  G_STMT_START {                                        \
