...
#include "rig/components/rig-camera.h"
...
static CoglPipeline *
create_1d_gaussian_blur_pipeline (RigContext *ctx, int n_taps)
{
...
{
  RigGaussianBlurrer *blurrer = g_slice_new0 (RigGaussianBlurrer);
...
static void
_rig_gaussian_blurrer_free_buffers (RigGaussianBlurrer *blurrer)
{
  if (blurrer->x_pass)
    {
...
{
  _rig_gaussian_blurrer_free_buffers (blurrer);
  rig_ref_countable_unref (blurrer->x_pass_camera);
  rig_ref_countable_unref (blurrer->y_pass_camera);
  g_slice_free (RigGaussianBlurrer, blurrer);
}
                           ...
                           CoglTexture *source)
{
  ...
  if (blurrer->x_pass_camera)
    rig_camera_set_framebuffer (blurrer->x_pass_camera, blurrer->x_pass_fb);
  else
    blurrer->x_pass_camera = rig_camera_new (blurrer->ctx, blurrer->x_pass_fb);

  if (blurrer->y_pass_camera)
    rig_camera_set_framebuffer (blurrer->y_pass_camera, blurrer->y_pass_fb);
  else
    blurrer->y_pass_camera = rig_camera_new (blurrer->ctx, blurrer->y_pass_fb);
  ...
  /* x pass */
  rig_camera_flush (blurrer->x_pass_camera);
  ...
  rig_camera_end_frame (blurrer->x_pass_camera);

  /* y pass */
  rig_camera_flush (blurrer->y_pass_camera);
  ...
  rig_camera_end_frame (blurrer->y_pass_camera);
