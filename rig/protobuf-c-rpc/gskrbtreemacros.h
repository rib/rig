 ...
 *         Find the first node in the tree which is after or equal to 'key'.
 *
 * XXX: how about rename INFIMUM, SUPREMUM as LOOKUP_LE, LOOKUP_GE *
 * respectively?
 ...
 * You can think of it as a lookup that allows "slip" in the direction
 * indicated.
 ...
 * but also keeps a "count" field at every node telling the size of that
 * subtree.
...
#define GSK_RBTREE_REMOVE(tree, node) GSK_RBTREE_REMOVE_(tree, node)
#define GSK_RBTREE_LOOKUP(tree, key, out) GSK_RBTREE_LOOKUP_(tree, key, out)
#define GSK_RBTREE_LOOKUP_COMPARATOR(tree, key, key_comparator, out)           \
    GSK_RBTREE_LOOKUP_COMPARATOR_(tree, key, key_comparator, out)

#define GSK_RBTREE_FIRST(tree, out) GSK_RBTREE_FIRST_(tree, out)
#define GSK_RBTREE_LAST(tree, out) GSK_RBTREE_LAST_(tree, out)
#define GSK_RBTREE_NEXT(tree, in, out) GSK_RBTREE_NEXT_(tree, in, out)
#define GSK_RBTREE_PREV(tree, in, out) GSK_RBTREE_PREV_(tree, in, out)

#define GSK_RBTREE_SUPREMUM(tree, key, out) GSK_RBTREE_SUPREMUM_(tree, key, out)
...
#define GSK_RBTREE_INFIMUM(tree, key, out) GSK_RBTREE_INFIMUM_(tree, key, out)
                ...
                if (_gsk_at->parent == _gsk_at->parent->parent->left) {        \
                    ...
                    if (_gsk_y != NULL && is_red(_gsk_y)) {                    \
                        ...
                        if (_gsk_at == _gsk_at->parent->right) {               \
                            ...
                            GSK_RBTREE_ROTATE_LEFT(                            \
                                top, type, parent, left, right, _gsk_at);      \
                        }                                                      \
                        set_is_red(_gsk_at->parent, 0);                        \
                        set_is_red(_gsk_at->parent->parent, 1);                \
                        GSK_RBTREE_ROTATE_RIGHT(top,                           \
                                                type,                          \
                                                parent,                        \
                                                left,                          \
                                                right,                         \
                    ...
                    if (_gsk_y != NULL && is_red(_gsk_y)) {                    \
                        ...
                        if (_gsk_at == _gsk_at->parent->left) {                \
                            _gsk_at = _gsk_at->parent;                         \
                            GSK_RBTREE_ROTATE_RIGHT(                           \
                                top, type, parent, left, right, _gsk_at);      \
                        }                                                      \
                        set_is_red(_gsk_at->parent, 0);                        \
                        set_is_red(_gsk_at->parent->parent, 1);                \
                        GSK_RBTREE_ROTATE_LEFT(top,                            \
                                               type,                           \
                                               parent,                         \
                                               left,                           \
                                               right,                          \
...
#define GSK_RBTREE_REMOVE_(top,                                                \
    ...
    {                                                                          \
        ...
        _gsk_rb_del_x =                                                        \
            _gsk_rb_del_y->left ? _gsk_rb_del_y->left : _gsk_rb_del_y->right;  \
        if (_gsk_rb_del_x)                                                     \
        ...
        if (_gsk_rb_del_fixup) {                                               \
            ...
            while (_gsk_rb_del_x != top &&                                     \
                ...
                type _gsk_rb_del_xparent = _gsk_rb_del_x                       \
                                           ? _gsk_rb_del_x->parent         \
                                           : _gsk_rb_del_nullpar;          \
                if (_gsk_rb_del_x == _gsk_rb_del_xparent->left) {              \
                    ...
                    if (!(_gsk_rb_del_w->left &&                               \
                          is_red(_gsk_rb_del_w->left)) &&                      \
                        !(_gsk_rb_del_w->right &&                              \
                          is_red(_gsk_rb_del_w->right))) {                     \
                        set_is_red(_gsk_rb_del_w, 1);                          \
                        _gsk_rb_del_x = _gsk_rb_del_xparent;                   \
                    } else {                                                   \
                        if (!(_gsk_rb_del_w->right &&                          \
                              is_red(_gsk_rb_del_w->right))) {                 \
                            if (_gsk_rb_del_w->left)                           \
                        ...
                        set_is_red(_gsk_rb_del_w,                              \
                                   is_red(_gsk_rb_del_xparent));               \
 ...
 */
#define GSK_RBTREE_SUPREMUM_COMPARATOR_(top,                                   \
                                        type,                                  \
                                        is_red,                                \
                                        set_is_red,                            \
                                        parent,                                \
...
#define GSK_RBTREE_LOOKUP_(                                                    \
    ...
    GSK_RBTREE_LOOKUP_COMPARATOR_(top,                                         \
                                  ...
                                  comparator,                                  \
                                  out)
#define GSK_RBTREE_SUPREMUM_(                                                  \
        top, type, is_red, set_is_red, parent, left, right, comparator, key, out)  \
    GSK_RBTREE_SUPREMUM_COMPARATOR_(top,                                       \
                                    type,                                      \
                                    is_red,                                    \
                                    set_is_red,                                \
                                    parent,                                    \
                                    ...
                                    comparator,                                \
                                    out)
#define GSK_RBTREE_INFIMUM_(                                                   \
        top, type, is_red, set_is_red, parent, left, right, comparator, key, out)  \
    GSK_RBTREE_INFIMUM_COMPARATOR_(top,                                        \
                                   type,                                       \
                                   is_red,                                     \
                                   set_is_red,                                 \
                                   parent,                                     \
...
#define GSK_RBTREE_LAST_(                                                      \
        top, type, is_red, set_is_red, parent, left, right, comparator, out)       \
    GSK_RBTREE_FIRST_(                                                         \
        top, type, is_red, set_is_red, parent, right, left, comparator, out)

/* --- RBC-Tree --- */
#define GSK_RBCTREE_INSERT(tree, node, collision_node)                         \
    GSK_RBCTREE_INSERT_(tree, node, collision_node)
#define GSK_RBCTREE_REMOVE(tree, node) GSK_RBCTREE_REMOVE_(tree, node)
#define GSK_RBCTREE_LOOKUP(tree, key, out) GSK_RBCTREE_LOOKUP_(tree, key, out)
...
#define GSK_RBCTREE_FIRST(tree, out) GSK_RBCTREE_FIRST_(tree, out)
#define GSK_RBCTREE_LAST(tree, out) GSK_RBCTREE_LAST_(tree, out)
#define GSK_RBCTREE_NEXT(tree, in, out) GSK_RBCTREE_NEXT_(tree, in, out)
#define GSK_RBCTREE_PREV(tree, in, out) GSK_RBCTREE_PREV_(tree, in, out)
...
#define GSK_RBCTREE_INFIMUM(tree, key, out) GSK_RBCTREE_INFIMUM_(tree, key, out)
    ...
    {                                                                          \
        ...
        if (!_gsk_rb_del_y->parent)                                            \
            ...
            if (_gsk_rb_del_y == _gsk_rb_del_y->parent->left)                  \
                ...
                _gsk_rb_del_y->parent->right = _gsk_rb_del_x;                  \
            _GSK_RBCTREE_FIX_COUNT_AND_UP(type,                                \
                                          parent,                              \
                                          left,                                \
                                          right,                               \
                                          get_count,                           \
                                          set_count,                           \
                                          _gsk_rb_del_y->parent);              \
        }                                                                      \
        _gsk_rb_del_fixup = !is_red(_gsk_rb_del_y);                            \
        if (_gsk_rb_del_y != _gsk_rb_del_z) {                                  \
            set_is_red(_gsk_rb_del_y, is_red(_gsk_rb_del_z));                  \
...
#define GSK_RBCTREE_LOOKUP_COMPARATOR_(top,                                    \
    ...
    GSK_RBTREE_LOOKUP_COMPARATOR_(top,                                         \
                                  ...
                                  key_comparator,                              \
                                  out)
#define GSK_RBCTREE_INFIMUM_COMPARATOR_(top,                                   \
                                        type,                                  \
                                        is_red,                                \
                                        set_is_red,                            \
                                        get_count,                             \
    ...
    GSK_RBTREE_INFIMUM_COMPARATOR_(top,                                        \
                                   ...
                                   key_comparator,                             \
                                   out)
#define GSK_RBCTREE_SUPREMUM_COMPARATOR_(top,                                  \
                                         type,                                 \
                                         is_red,                               \
                                         set_is_red,                           \
                                         get_count,                            \
    ...
    GSK_RBTREE_SUPREMUM_COMPARATOR_(top,                                       \
                                    ...
                                    key_comparator,                            \
                                    out)
#define GSK_RBCTREE_LOOKUP_(top,                                               \
                            type,                                              \
                            is_red,                                            \
                            set_is_red,                                        \
                            get_count,                                         \
    ...
    GSK_RBTREE_LOOKUP_COMPARATOR_(top,                                         \
                                  ...
                                  comparator,                                  \
                                  out)
#define GSK_RBCTREE_SUPREMUM_(top,                                             \
                              type,                                            \
                              is_red,                                          \
                              set_is_red,                                      \
                              get_count,                                       \
    ...
    GSK_RBTREE_SUPREMUM_COMPARATOR_(top,                                       \
                                    ...
                                    comparator,                                \
                                    out)
#define GSK_RBCTREE_INFIMUM_(top,                                              \
                             type,                                             \
                             is_red,                                           \
                             set_is_red,                                       \
                             get_count,                                        \
...
#define GSK_RBCTREE_NEXT_(top,                                                 \
    ...
    GSK_RBTREE_NEXT_(top,                                                      \
                     ...
                     in,                                                       \
                     out)
#define GSK_RBCTREE_PREV_(                                                     \
        top, type, is_red, set_is_red, parent, left, right, comparator, in, out)   \
    GSK_RBTREE_PREV_(top,                                                      \
                     type,                                                     \
                     is_red,                                                   \
                     set_is_red,                                               \
                     parent,                                                   \
...
#define GSK_RBCTREE_GET_BY_INDEX_(top,                                         \
                                  type,                                        \
                                  is_red,                                      \
                                  set_is_red,                                  \
                                  get_count,                                   \
    ...
    {                                                                          \
        if (top == NULL || (index) >= get_count(top))                          \
            ...
            GSK_RBCTREE_GET_BY_INDEX_UNCHECKED_(top,                           \
                                                type,                          \
                                                is_red,                        \
                                                set_is_red,                    \
                                                get_count,                     \
...
#define GSK_RBCTREE_GET_BY_INDEX_UNCHECKED_(top,                               \
    ...
    {                                                                          \
        ...
        for (;; ) {                                                             \
            guint _gsk_left_size =                                             \
                _gsk_at->left ? get_count(_gsk_at->left) : 0;                  \
...
#define GSK_RBCTREE_ROTATE_LEFT(                                               \
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            guint _gsk_rot_n1 =                                                \
                _gsk_rot_y->right ? get_count(_gsk_rot_y->right) : 0;          \
