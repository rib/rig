...
#define GSK_STACK_FOREACH(stack, iter_var, code)                               \
    GSK_STACK_FOREACH_(stack, iter_var, code)
#define GSK_STACK_SORT(stack, comparator) GSK_STACK_SORT_(stack, comparator)
#define GSK_STACK_GET_BOTTOM(stack, rv_node)                                   \
    GSK_STACK_GET_BOTTOM_(stack, rv_node)

#define GSK_STMT_START do
#define GSK_STMT_END while (0)
...
#define GSK_STACK_INSERT_AFTER_(type, top, next, above_node, new_node)         \
    ...
    GSK_STMT_END
#define GSK_STACK_IS_EMPTY_(type, top, next) ((top) == NULL)
...
#define GSK_QUEUE_IS_EMPTY_(type, head, tail, next) ((head) == NULL)
...
#define GSK_LIST_INSERT_AFTER(list, at, node)                                  \
    GSK_LIST_INSERT_AFTER_(list, at, node)
#define GSK_LIST_INSERT_BEFORE(list, at, node)                                 \
    GSK_LIST_INSERT_BEFORE_(list, at, node)
...
#define GSK_LIST_IS_EMPTY_(type, first, last, prev, next) ((first) == NULL)
