...
_rig_downsampler_reset (RigDownsampler *downsampler)
{
  ...
  if (downsampler->camera)
    {
      rig_ref_countable_unref (downsampler->camera);
                            ...
                            int scale_factor_y)
{
  ...
  if (downsampler->dest == NULL ||
      ...
      cogl_texture_get_format (downsampler->dest) != format)
    {
      ...
      downsampler->camera = rig_camera_new (downsampler->ctx, downsampler->fb);
      rig_camera_set_near_plane (downsampler->camera, -1.f);
      rig_camera_set_far_plane (downsampler->camera, 1.f);
    }

  pipeline = cogl_pipeline_copy (downsampler->pipeline);
  cogl_pipeline_set_layer_texture (pipeline, 0, source);

  rig_camera_flush (downsampler->camera);
  ...
  rig_camera_end_frame (downsampler->camera);
