...
#define MOD(sp) ((sp) % USCRIPT_PAREN_STACK_DEPTH)
#define LIMIT_INC(sp)                                                          \
    (((sp) < USCRIPT_PAREN_STACK_DEPTH) ? (sp) + 1 : USCRIPT_PAREN_STACK_DEPTH)
#define INC(sp, count) (MOD((sp) + (count)))
#define INC1(sp) (INC(sp, 1))
#define DEC(sp, count) (MOD((sp) + USCRIPT_PAREN_STACK_DEPTH - (count)))
...
static const UChar32 pairedChars[] = { 0x0028, 0x0029, /* ascii paired
                                                          punctuation */
                                       0x003c, 0x003e, 0x005b, 0x005d, 0x007b,
                                       0x007d, 0x00ab, 0x00bb, /* guillemets */
                                       0x2018, 0x2019, /* general punctuation */
                                       0x201c, 0x201d, 0x2039, 0x203a, 0x3008,
                                       0x3009, /* chinese paired punctuation */
                                       0x300a, 0x300b, 0x300c, 0x300d, 0x300e,
                                       0x300f, 0x3010, 0x3011, 0x3014, 0x3015,
                                       0x3016, 0x3017, 0x3018, 0x3019, 0x301a,
                                       0x301b };

static void
push(UScriptRun *scriptRun, int32_t pairIndex, UScriptCode scriptCode)
{
...
static void
fixup(UScriptRun *scriptRun, UScriptCode scriptCode)
{
...
{
    return scriptOne <= USCRIPT_INHERITED || scriptTwo <= USCRIPT_INHERITED ||
           scriptOne == scriptTwo;
}

U_CAPI UBool U_EXPORT2
rig_uscript_initRun(UScriptRun *runner,
                    const UChar *src,
                    int32_t length,
                    UErrorCode *pErrorCode)
{
    ...
    rig_uscript_setRunText(runner, src, length, pErrorCode);
...
rig_uscript_closeRun(UScriptRun *runner)
{
    /* NOP */
}

U_CAPI void U_EXPORT2
rig_uscript_resetRun(UScriptRun *scriptRun)
{
    if (scriptRun != NULL) {
...
U_CAPI void U_EXPORT2
rig_uscript_setRunText(UScriptRun *scriptRun,
                       const UChar *src,
                       int32_t length,
                       UErrorCode *pErrorCode)
{
    ...
    rig_uscript_resetRun(scriptRun);
}

U_CAPI UBool U_EXPORT2
rig_uscript_nextRun(UScriptRun *scriptRun,
                    int32_t *pRunStart,
                    int32_t *pRunLimit,
                    UScriptCode *pRunScript)
{
    ...
    for (scriptRun->scriptStart = scriptRun->scriptLimit;
         scriptRun->scriptLimit < scriptRun->textLength;
        ...
        if (high >= 0xD800 && high <= 0xDBFF &&
            scriptRun->scriptLimit < scriptRun->textLength - 1) {
        ...
        if (pairIndex >= 0) {
            if ((pairIndex & 1) == 0) {
                ...
                while (STACK_IS_NOT_EMPTY(scriptRun) &&
                       TOP(scriptRun).pairIndex != pi) {
                    pop(scriptRun);
                }
        ...
        if (sameScript(scriptRun->scriptCode, sc)) {
            if (scriptRun->scriptCode <= USCRIPT_INHERITED &&
                sc > USCRIPT_INHERITED) {
