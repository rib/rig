...
#define USCRIPT_PAREN_STACK_DEPTH 32
...
struct UScriptRun {
    ...
    struct ParenStackEntry parenStack[USCRIPT_PAREN_STACK_DEPTH];
    int32_t parenSP;
    int32_t pushCount;
    int32_t fixupCount;
};
...
/**
 * Initializes a <code>UScriptRun</code> object for iterating over the given
 * text. This object must
 * be cleaned up using <code>uscript_closeRun()</code>. Note that this object
 * does not copy the source text,
 * only the pointer to it. You must make sure that the pointer remains valid
 * until you call
 ...
 *
 * @param pErrorCode is a pointer to a valid <code>UErrorCode</code> value. If
 * this value
 ...
 *
 * @return the address of <code>UScriptRun</code> object which will iterate over
 * the text,
 *         or <code>NULL</code> if the operation failed.
 */
U_CAPI UBool U_EXPORT2 rig_uscript_initRun(UScriptRun *runner,
                                           const UChar *src,
                                           int32_t length,
                                           UErrorCode *pErrorCode);

/**
 * Frees the given <code>UScriptRun</code> object and any storage associated
 * with it.
 * On return, scriptRun no longer points to a valid <code>UScriptRun</code>
 * object.
 *
 * @param scriptRun is the <code>UScriptRun</code> object which will be freed.
 */
U_CAPI void U_EXPORT2 rig_uscript_closeRun(UScriptRun *scriptRun);

/**
 * Reset the <code>UScriptRun</code> object so that it will start iterating from
 * the beginning.
 *
 * @param scriptRun is the address of the <code>UScriptRun</code> object to be
 * reset.
 */
U_CAPI void U_EXPORT2 rig_uscript_resetRun(UScriptRun *scriptRun);
 ...
 *
 * @param src is the address of the new array of characters over which to
 * iterate.
 *        If <code>src == NULL</code> and <code>length == 0</code>,
 ...
 *        On exit the value will indicate the success of the operation.
 */
U_CAPI void U_EXPORT2 rig_uscript_setRunText(UScriptRun *scriptRun,
                                             const UChar *src,
                                             int32_t length,
                                             UErrorCode *pErrorCode);

/**
 * Advance the <code>UScriptRun</code> object to the next script run, return the
 * start and limit
 * offsets, and the script of the run.
 *
 * @param scriptRun is the address of the <code>UScriptRun</code> object.
 *
 * @param pRunStart is a pointer to the variable to receive the starting offset
 * of the next run.
 *        This pointer can be <code>NULL</code> if the value is not needed.
 *
 * @param pRunLimit is a pointer to the variable to receive the limit offset of
 * the next run.
 *        This pointer can be <code>NULL</code> if the value is not needed.
 *
 * @param pRunScript is a pointer to the variable to receive the UScriptCode for
 * the
 *        script of the current run. This pointer can be <code>NULL</code> if
 * the value is not needed.
 *
 * @return true if there was another script run.
 */
U_CAPI UBool U_EXPORT2 rig_uscript_nextRun(UScriptRun *scriptRun,
                                           int32_t *pRunStart,
                                           int32_t *pRunLimit,
                                           UScriptCode *pRunScript);
