...
/* Note: Our use case for this code is to provide a locale independent
 * snprint() that will consistently use '.' as a decimal point for
...
/*
 * We explicitly want to make sure to avoid referring
 * to the locale for what decimal point character to
 * use and consistently use '.'
 */
...
#if HAVE_STDARG_H
int
c_ascii_snprintf(char *str, size_t size, const char *format, ...)
#else
int
c_ascii_snprintf(va_alist) va_dcl
#endif	/* HAVE_STDARG_H */
{
#if !HAVE_STDARG_H
	char *str;
