...
static void gst_mem_src_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_mem_src_get_property (GObject * object, guint prop_id,
...
gst_mem_src_class_init (GstMemSrcClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_MEMORY,
      g_param_spec_pointer ("memory", "Memory Address",
          "Address of memory to read",
          G_PARAM_READWRITE | GST_PARAM_MUTABLE_READY));
  g_object_class_install_property (gobject_class, PROP_MEMORY,
      g_param_spec_uint64 ("size", "Memory Size",
          "Length of memory region", 0, G_MAXUINT64, 0,
          G_PARAM_READWRITE | GST_PARAM_MUTABLE_READY));
...
static void
gst_mem_src_set_property (GObject * object, guint prop_id,
                          const GValue * value, GParamSpec * pspec)
{
  GstMemSrc *src;
...
static void
gst_mem_src_get_property (GObject * object, guint prop_id, GValue * value,
                          GParamSpec * pspec)
{
  GstMemSrc *src;
...
static gboolean
gst_mem_src_uri_set_uri (GstURIHandler * handler, const gchar * uri,
                         GError ** err)
{
  GstMemSrc *src = GST_MEM_SRC (handler);
  gchar *protocol;
